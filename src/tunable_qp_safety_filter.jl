...
"""
function TunableQPSafetyFilter(cbfs::Vector{ControlBarrierFunction}, Σ::ControlAffineSystem, kd::Function)
    try
        ...
        return TunableQPSafetyFilter((x,t) -> solve_time_varying_tunable_cbf_qp(x, t, Σ, cbfs, kd))
    end
end
...
"""
TunableQPSafetyFilter(cbf::ControlBarrierFunction, Σ::ControlAffineSystem, kd::Function) = TunableQPSafetyFilter([cbf], Σ, kd)
...
"""
function solve_tunable_cbf_qp(x, Σ::ControlAffineSystem, cbfs::Vector{ControlBarrierFunction}, kd::Function)
    model = Model(OSQP.Optimizer)
    set_silent(model)
    u = Σ.m == 1 ? @variable(model, u) : @variable(model, u[1:Σ.m])
    @variable(model, αs[1:length(cbfs)])
    @objective(model, Min, 0.5*(u - kd(x))'*(u - kd(x)) + sum([(α - 1.0)^2 for α in αs]))
...
"""
function solve_time_varying_tunable_cbf_qp(x, t, Σ::ControlAffineSystem, cbfs::Vector{ControlBarrierFunction}, kd::Function)
    model = Model(OSQP.Optimizer)
    set_silent(model)
    u = Σ.m == 1 ? @variable(model, u) : @variable(model, u[1:Σ.m])
    @variable(model, αs[1:length(cbfs)])
    @objective(model, Min, 0.5*(u - kd(x,t))'*(u - kd(x,t)) + sum([(α - 1.0)^2 for α in αs]))
    for (cbf, α) in zip(cbfs, αs)
        @constraint(model, cbf.Lfh(x) + cbf.Lgh(x)*u ≥ -α*cbf.α(cbf(x)))
    end
    optimize!(model)
