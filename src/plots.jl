...
function plot_vector_field(xs, ys, f::Function; scale=0.15, lw=1)
    ...
    df(x, y) = scale * normalize(f(x, y))
    c = vector_field_colors(Xs, Ys, f)
    
    return quiver(Xs, Ys, quiver=df, line_z=c, lw=lw, c=:viridis, colorbar=true)
end
...
function plot_vector_field!(xs, ys, f::Function; scale=0.15, lw=1)
    ...
    return quiver!(fig, Xs, Ys, quiver=df, line_z=c, lw=lw, c=:viridis, colorbar=true)
end
