...
function CBFQP(Σ::ControlAffineSystem, HOCBF::HighOrderCBF, CLF::ControlLyapunovFunction, A, b, p::Float64=10e2)
	function control(x)
		model = Model(OSQP.Optimizer)
    	set_silent(model)
    	Σ.m == 1 ? @variable(model, u) : @variable(model, u[1:Σ.m])
...
function CBFQP(Σ::ControlAffineSystem, HOCBFs::Vector{HighOrderCBF}, κ::FeedbackPolicy, A, b)
	function control(x)
		model = Model(OSQP.Optimizer)
    	set_silent(model)
    	Σ.m == 1 ? @variable(model, u) : @variable(model, u[1:Σ.m])
...
function CBFQP(Σ::ControlAffineSystem, HOCBFs::Vector{HighOrderCBF}, CLF::ControlLyapunovFunction, A, b, p::Float64=10e2)
	function control(x)
		model = Model(OSQP.Optimizer)
    	set_silent(model)
    	Σ.m == 1 ? @variable(model, u) : @variable(model, u[1:Σ.m])
